...
void   varifold(double *, double *, double *, fcndata &);
...
void objgrd(double *h_objPtr, double *d_grdGrpStk, double *d_ctlGrpStk, fcndata &fcnObj)
{
	...
	cudaMemset(fcnObj.d_ctlCumStk, 0, sizeof(double) * rgdNdeNum);
	cudaMemcpy(fcnObj.d_ctlCumStk + rgdNdeNum, fcnObj.prm.d_cenIniMat,
	...
	*h_objPtr = computeControlCost(d_ctlGrpStk, fcnObj.prm.d_wgtGrpVec, fcnObj);
	...
	double h_vfdVal;
	varifold(&h_vfdVal, fcnObj.d_dqVfdMat, d_rgdNdeEndMat, fcnObj);

	*h_objPtr = timeStp * (*h_objPtr) + fcnObj.prm.tgtWgt * h_vfdVal;

	double *d_ctlCumEndMat = fcnObj.d_ctlCumStk + (timeNum - 1) * rgdNdeNum * RGDDOF;
	dqRigidDeformation(fcnObj.d_pMat, d_ctlCumEndMat, fcnObj.d_dqVfdMat, fcnObj);
